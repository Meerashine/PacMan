...
class Game(object):
    ...
    def run_logic(self):
        ...
        if not self.game_over:
            ...
            if len(block_hit_list) > 0:
                ...
                if self.score % 10 == 0:
                    self.reward += 1
                    self.slime_count = True
                    if self.reward > 2:
                        self.enemies.empty()
                        self.enemies = pygame.sprite.Group()
                        self.enemies.add(Slime(288,100,0,2))
                        self.enemies.add(Slime(288,320,0,-2))
                        self.enemies.add(Slime(544,128,0,2))
    ...
    def display_frame(self,screen):
        ...
        if self.game_over:
            ...
            text_reward = self.font.render("Life length: " + str(self.reward),True,GREEN)
                   
            # Put the text on the screen
            screen.blit(text,[120,20])
            screen.blit(text_reward,[350,20])
            """if self.reward > 2:
                self.enemies.empty()
                self.enemies = pygame.sprite.Group()
                self.enemies.add(Slime(288,100,0,2))
                ...
                #self.enemies.add(Slime(32,224,0,2))
                #color = 255, 0, 0
                #rect1 =random.randrange(SCREEN_WIDTH)
                #rect1 =random.randrange(SCREEN_HEIGHT)
                #rect=(rect1,rect2)
                #enemy_list =self.enemies.sprites()
                #print(enemy_list)
                
                #rem =random.choice(self.enemies.sprites())
                #print(rem)
                #for entity in self.enemies:
                    #print(entity)
                    #screen.blit(entity.surf, entity.rect)
                    #self.image = pygame.image.load(r"C:\Users\mejoe\Downloads\Pacman-In-Python-Code\Pacman In Python Code\PacMan-Python-master\PacMan\player.png").convert_alpha()
                    #self.enemies.setTexture(self.image)
                    #
                    #self.enemies.remove(Slime(288,96,0,2,self.slime_count))
                    #self.enemies.remove(Slime(288,96,0,2,self.slime_count))
                    #self.enemies.remove(Slime(448,64,-2,0,self.slime_count))
                    #self.enemies.remove(Slime(640,448,2,0,self.slime_count))
                    #self.enemies.remove(Slime(448,320,2,0,self.slime_count))
...
class Menu(object):
    state = 0
    def __init__(self,items,font_color=(0,0,0),select_color=(255,0,0),ttf_font='Helvetica',font_size=25):
